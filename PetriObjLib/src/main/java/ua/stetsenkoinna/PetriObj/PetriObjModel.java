...
public class PetriObjModel implements Serializable, Cloneable {
    ...
    public PetriSim findObj(String nameSub) {
        for (var o : listObj) {
            if (o.getName().contains(nameSub)) {
                return o;
            }
        }
        return null;
    ...
    public boolean moveNext(double timeModeling) {

        boolean isMinSetInPreviousMethodCall = true;
        if (nextT == 0) {
            isMinSetInPreviousMethodCall = false;
        ...
        while (this.getCurrentTime() < this.getSimulationTime()) {
            ...
            conflictObj.clear();
            if (isMinSetInPreviousMethodCall) {
                min = nextT;
                isMinSetInPreviousMethodCall = false;
            } else {
        ...
        nextT = min;
        return this.getCurrentTime() < this.getSimulationTime();
    }
    ...
    public void go(double timeModeling) {
        ...
        while (this.getCurrentTime() < this.getSimulationTime()) {
            ...
            if (prevT != this.getCurrentTime() && isProtocolPrint()) {
